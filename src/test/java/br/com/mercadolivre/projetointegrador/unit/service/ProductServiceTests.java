...
public class ProductServiceTests {
    ...
    @Test
    @DisplayName(
            "Given a valid product, when call createProduct, then createProduct must be called"
                + " once.")
    ...
    @Test
    @DisplayName(
            "Given an existing product, when call find method with id from that product, then"
                + " return this one")
    ...
    @Test
    @DisplayName(
            "Given an id from a non existing product, when call find by id method, then throw an"
                + " error")
    public void shouldThrownNotFoundExceptionWhenIdDoesNotExists() {

        Exception thrown =
                Assertions.assertThrows(
                        NotFoundException.class, () -> productService.findById(10L));
    ...
    @Test
    @DisplayName(
            "Given an valid id and valid updated information, when call updateProduct, then"
                + " updateProduct must be called once.")
    public void shouldUpdateProduct() throws NotFoundException {
        ...
        Mockito.when(productRepository.findById(1L))
                .thenReturn(Optional.ofNullable(newTestProduct));
