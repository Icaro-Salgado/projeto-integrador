...
public class BatchServiceTests {
    ...
    @Test
    @DisplayName(
            "Given a valid batch with existing product, when call createBatch, then"
                + " batchRepository.save should be called once;")
    ...
    public void failsToCreateBatchWithNonExistingProduct() {
        ...
        NotFoundException thrown =
                Assertions.assertThrows(
                        NotFoundException.class, () -> batchService.createBatch(batch));
    ...
    @Test
    @DisplayName(
            "Given a list of batches containing 3 batches, when call findAll, then should return"
                + " size equals 3")
    ...
    @Test
    @DisplayName(
            "Given an existing Batch, when call updateBatch, then should update all properties")
    ...
    @Test
    @DisplayName(
            "Given a non existing batch, when call updateBatch, then throws an"
                + " NotFoundException(\"Lote não encontrado\");")
    ...
    @Test
    @DisplayName(
            "Given a existing batch, when call deleteBatch, then should call batchRepository.delete"
                + " once with this batch")
    ...
    @Test
    @DisplayName(
            "Given a non existing batch, when call deleteBatch, then throws an"
                + " NotFoundException(\"Lote não encontrado\");")
