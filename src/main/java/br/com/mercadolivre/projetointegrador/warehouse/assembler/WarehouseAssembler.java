...
public class WarehouseAssembler {
    ...
    public ResponseEntity<WarehouseResponseDTO> toResponse(Warehouse entity, HttpStatus status, HttpHeaders headers) {
        WarehouseResponseDTO dto = WarehouseMapper.INSTANCE.toResponseDTO(entity);

        Links links =
                Links.of(linkTo(methodOn(WarehouseController.class).findById(entity.getId())).withSelfRel());
